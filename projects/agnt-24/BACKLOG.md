...
- [ ] **Важно**: сейчас в приложении нет способа добавить второго человека
      в агентство — `auth.js` умеет только создать НОВОЕ агентство с ролью
      admin. Таблица `invites` есть в schema.sql, но никакой UI/логика к ней
      не подключены. Без этого роль 'mop' некому назначить и вся фича переноса
      объектов нетестируема на реальных пользователях (только вручную через
      Supabase Dashboard: `UPDATE profiles SET role='mop' WHERE id=...`)
- [ ] Экран/флоу принятия инвайта (токен из `invites.token`) → создание
      profile с `agency_id` пригласившего и заданной ролью
